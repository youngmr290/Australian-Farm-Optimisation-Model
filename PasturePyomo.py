...
def paspyomo_local():
    ...
    try:
        ...
        pass
    model.p_erosion = Param(model.s_feed_periods, model.s_lmus, model.s_pastures, initialize=pas.p_erosion_flt, default=0, doc='erosion limit in each period')
    ...
    try:
        ...
        pass
    model.p_poc_con = Param(model.s_feed_periods ,model.s_lmus, model.s_pastures, initialize=pas.poc_con(),default=0, doc='consumption of pasture on 1ha of a crop paddock each day for each lmu in each feed period')
    ...
    try:
        ...
        pass
    model.p_poc_md = Param(model.s_feed_periods, model.s_pastures, initialize=pas.poc_md(),default=0, doc='md of pasture on crop paddocks for each feed period')
    ...
    try:
        ...
        pass
    model.p_poc_vol = Param(model.s_feed_periods, model.s_pastures, initialize=pas.poc_vol(),default=0, doc='vol (ri intake) of pasture on crop paddocks for each feed period')
    ...
    model.con_greenpas = Constraint(model.s_feed_periods, model.s_lmus, model.s_pastures, rule = greenpas, doc='green pasture available on each soil type in each feed period')
    ...
    model.con_drypas = Constraint(model.s_dry_groups, model.s_feed_periods, model.s_pastures, rule = drypas, doc='High and low quality dry pasture available in each period')
    ...
    model.con_pasarea = Constraint(model.s_feed_periods, model.s_lmus, model.s_pastures, rule = pasarea, doc='Pasture area row for growth constraint on each soil for each feed period (ha)')
    ...
    model.con_erosion = Constraint(model.s_pastures, rule = erosion, doc='green pasture available on each soil type in each feed period')
...
model.v_poc = Var(model.s_sheep_pools, model.s_feed_periods, model.s_lmus, bounds = (0,None) , doc='tonnes of poc consumed by each sheep pool in each period on each lmu')
...
def pas_md(model,e,f):
    return sum(model.v_greenpas_ha[e,g,o,f,l,t] * p_me_cons_grnha[e,g,o,f,l,t] + model.v_drypas_consumed[e,d,f,t] * model.p_dry_mecons_t[e,d,f,t] \
...
def pas_vol(model,e,f):
    return sum(model.v_greenpas_ha[e,g,o,f,l,t] * p_volume_grnha[g,o,f,l,t] + model.v_drypas_consumed[e,d,f,t] * model.p_dry_volume_t[d,f,t] \
