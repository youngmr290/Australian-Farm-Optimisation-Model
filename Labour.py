...
def labour_general():
    ...
    for i in range(len(labour_periods['date'])-1):
        ...
        ###if labour i period starts before leave starts and leave finishes before the labour period finished then that period gets all the leave.
            ...
            labour_periods.loc[i , 'manager leave'] = labour_input_data['leave_manager']
        ###if labour i period starts before leave starts and leave finishes after the labour period finished then that period gets leave from the start date of leave to the end of the labour period.
            ...
            labour_periods.loc[i , 'manager leave'] = labour_periods.loc[i + 1 , 'date'] - labour_input_data['leave_manager_start_date']            
        ###if labour i period starts after leave starts and leave finishes before the labour period finished then that period gets leave from the beginning on the labour period to the end date of leave.
            ...
            labour_periods.loc[i , 'manager leave'] = labour_input_data['leave_manager_start_date'] + labour_input_data['leave_manager'] - labour_periods.loc[i , 'date']             
    ...
    for i in labour_periods['date']: #loops through each period date
        ...
        if i in per.period_dates(per.wet_seeding_start_date(),pinp.crop['seed_period_lengths']): #checks if the date is a seed period
            ...
            * labour_input_data['manager_hours']['seeding'] + labour_periods.loc[labour_periods['date']==i , 'manager weekend'] / datetime.timedelta(days=1) \
            * labour_input_data['manager_hours']['seeding'] #convert the datetime into a float by dividing number of days by 1 day, then multiply by number of hours that can be worked during seeding.
        elif i in per.period_dates(pinp.crop['harv_date'],pinp.crop['harv_period_lengths']):
            labour_periods.loc[labour_periods['date']==i , 'manager hours'] = labour_periods.loc[labour_periods['date']==i , 'manager weekdays'] / datetime.timedelta(days=1) \
            * labour_input_data['manager_hours']['harvest'] + labour_periods.loc[labour_periods['date']==i , 'manager weekend'] / datetime.timedelta(days=1) \
            * labour_input_data['manager_hours']['harvest']  
        else:
            labour_periods.loc[labour_periods['date']==i , 'manager hours'] = labour_periods.loc[labour_periods['date']==i , 'manager weekdays'] / datetime.timedelta(days=1) \
            * labour_input_data['manager_hours']['weekdays'] + labour_periods.loc[labour_periods['date']==i , 'manager weekend'] / datetime.timedelta(days=1) \
            * labour_input_data['manager_hours']['weekends']  
        ...
        if i in per.period_dates(per.wet_seeding_start_date(),pinp.crop['seed_period_lengths']): #checks if the date is a seed period
            ...
            * labour_input_data['permanent_hours']['seeding'] #convert the datetime into a float by dividing number of days by 1 day, then multiply by number of hours that can be worked during seeding 
        elif i in per.period_dates(pinp.crop['harv_date'],pinp.crop['harv_period_lengths']):
            labour_periods.loc[labour_periods['date']==i , 'permanent hours'] = labour_periods.loc[labour_periods['date']==i , 'permanent weekdays'] / datetime.timedelta(days=1) \
        ...
        if i in per.period_dates(per.wet_seeding_start_date(),pinp.crop['seed_period_lengths']): #checks if the date is a seed period
            ...
            * labour_input_data['casual_hours']['seeding'] #convert the datetime into a float by dividing number of days by 1 day, then multiply by number of hours that can be worked during seeding.
        elif i in per.period_dates(pinp.crop['harv_date'],pinp.crop['harv_period_lengths']):
            labour_periods.loc[labour_periods['date']==i , 'casual hours'] = labour_periods.loc[labour_periods['date']==i , 'casual weekdays'] / datetime.timedelta(days=1) \
        ...
        or i in per.period_dates(pinp.crop['harv_date'],pinp.crop['harv_period_lengths']): #checks if the date is a seed period or harvest date
            labour_periods.loc[labour_periods['date']==i , 'casual ub'] =  (labour_input_data['max number casual labour harv seed'] )
        else:
            labour_periods.loc[labour_periods['date']==i , 'casual ub'] = (labour_input_data['max number casual labour normal'] )
        ###lower bound
        if i in per.period_dates(per.wet_seeding_start_date(),pinp.crop['seed_period_lengths']) \
        or i in per.period_dates(pinp.crop['harv_date'],pinp.crop['harv_period_lengths']): #checks if the date is a seed period or harvest date
            labour_periods.loc[labour_periods['date']==i , 'casual lb'] =  (labour_input_data['min number casual labour harv seed'] )
        else:
            labour_periods.loc[labour_periods['date']==i , 'casual lb'] = (labour_input_data['min number casual labour normal'] )
...
def perm_cost():
    ...
    + labour_input_data['permanent_cost'] * labour_input_data['permanent_workers_comp'] + labour_input_data['permanent_cost'] * labour_input_data['permanent_ls_leave']) / len(uinp.structure['cashflow_periods'])

    
#manager cost per cashflow period
def manager_cost():
    return labour_input_data['manager_cost'] / len(uinp.structure['cashflow_periods'])
