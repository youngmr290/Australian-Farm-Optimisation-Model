...
#define parameters
'''
#seeding rate on each lmu for each crop (ha/day)
model.p_seeding_rate = Param(model.s_crops, model.s_lmus, initialize=mac.overall_seed_rate(), default = 0.0, doc='rate of seeding ha/day provided by one crop gear')
#seed period length
model.p_wetseedperiod_length = Param(model.s_periods, initialize=mac.seed_days()['seed_days'].to_dict(), default = 0.0, doc='number of seeding days in each period')
#seeding cost ($/ha)
...
#contract harv rate for each crop (t/hr) (period constraints don't exist)
...
#pasture grazing days per feed period provided by 1ha of seeding in each seed period
...
model.v_seeding_machdays = Var(model.s_periods, model.s_crops, model.s_lmus, bounds=(0,None), doc='number of ha of each rotation')
#number of ha seeded using contractor
model.v_contractseeding_ha = Var(model.s_periods, model.s_crops, model.s_lmus, bounds=(0,None), doc='number of ha contract seeding for each crop')
...
model.v_harv_hours = Var(model.s_periods, model.s_crops, bounds=(0,None), doc='number of hours of harvesting')
#number of contract hours harvesting for each crop
model.v_contractharv_hours = Var(model.s_crops, bounds=(0,None), doc='number of contract hours of harvesting')
#tonnes of hay made
...
###################################
#function to determine sow supply, this will be passed to core model
def sow_supply(model,k,l,p):
    contract_seed = model.v_contractseeding_ha[p,k,l]
    farmer_seed = model.p_seeding_rate[k,l] * model.v_seeding_machdays[p,k,l] 
    return contract_seed + farmer_seed
...
#function to determine the tonnes that can be harvested, this will be passed to core model
...
def seeding_cost(model,c):
    #contract cost
    contract_cost[c] = sum(sum(sum(model.v_contractseeding_ha[p,k,l] * model.p_contract_seeding_cost[c] for l in model.s_lmus) for p in model.s_periods) for k in model.s_crops) 
    #cost per ha x number of days seeding x ha per day
    seeding_cost[c] = sum(sum(sum(model.p_seeding_cost[c,l] * model.v_seeding_machdays[p,k,l] * model.p_seeding_rate[k,l] for l in model.s_lmus) for p in model.s_periods) for k in model.s_crops)  
    return contract_cost[c] + seeding_cost[c]
 
#function to determine harv cost, this will be passed to core model
def harvesting_cost(model,c):
    #contract cost total
    contract_cost[c] = sum(model.v_contractharv_hours[k] * model.p_contractharv_cost[c,k] for k in model.s_crops)
    #cost per hr x number of hours
    farmer_harv_cost[c] = sum(sum(model.p_harv_cost[c,k] * model.v_harv_hours[p, k] for k in model.s_crops) for p in model.s_periods)
    return farmer_harv_cost[c] + contract_cost[c]

#includes hay cost
def mach_cost(model,c):
    hay_cost[c] = model.v_hay_made * model.p_contracthay_cost[c]
    return harvesting_cost(model,c) + seeding_cost(model,c) + hay_cost[c]
...
def total_dep(model):
    ...
    harv_dep= mac.harvest_dep() * sum(sum(model.v_harv_hours[p ,k] for k in model.s_crops) for p in model.s_periods)
...
def seed_period_days(model,p):
    ...
    model.p_wetseedperiod_length[p] * pinp.mach['number_crop_gear'] * pinp.mach['seeding_occur']
...
def harv_hours_limit(model, p):
    return sum(model.v_harv_hours[p, k] for k in model.s_crops) <= model.p_harv_hrs_max[p] * pinp.mach['number_crop_gear']
