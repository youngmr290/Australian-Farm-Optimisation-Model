...
def machpyomo_local(params, model):
    ...
    model.v_seeding_machdays = pe.Var(model.s_labperiods, model.s_landuses, model.s_lmus, model.s_season_types, bounds=(0,None), doc='number of days of seeding')
    ...
    model.v_seeding_crop = pe.Var(model.s_labperiods, model.s_landuses, model.s_lmus, model.s_season_types, bounds=(0,None), doc='number of ha of crop seeded')
    #number of ha seeded using contractor
    model.v_contractseeding_ha = pe.Var(model.s_labperiods, model.s_landuses, model.s_lmus, model.s_season_types, bounds=(0,None), doc='number of ha contract seeding for each crop')
    ...
    model.v_harv_hours = pe.Var(model.s_labperiods, model.s_harvcrops, model.s_season_types, bounds=(0,None), doc='number of hours of harvesting')
    #number of contract hours harvesting for each crop
    model.v_contractharv_hours = pe.Var(model.s_harvcrops, model.s_season_types, bounds=(0,None), doc='number of contract hours of harvesting')
    #tonnes of hay made
...
def f_con_seed_period_days(model):
    ...
    def seed_period_days(model,p,z):
        return sum(sum(model.v_seeding_machdays[p,k,l,z] for k in model.s_crops)for l in model.s_lmus) <= \
...
def f_con_harv_hours_limit(model):
    ...
    def harv_hours_limit(model, p, z):
        return sum(model.v_harv_hours[p,k,z] for k in model.s_harvcrops) <= model.p_harv_hrs_max[p,z] * model.p_number_harv_gear
...
def f_con_sow_supply(model):
    ...
    def sow_supply(model,p,k1,l,z):
        return -model.v_contractseeding_ha[p,k1,l,z] * model.p_contractseeding_occur[p,z] - model.p_seeding_rate[k1,l] * model.v_seeding_machdays[p,k1,l,z]   \
...
def ha_days_pasture_crop_paddocks(model,f,l,z):
    ...
    ha_days_contract= sum(sum(model.p_seeding_grazingdays[f,p,z] * model.v_contractseeding_ha[p,k,l,z] for k in model.s_crops) for p in model.s_labperiods)
    ##number of grazable pasture ha provided by farmer seeding
    ha_days_personal= sum(sum(model.p_seeding_grazingdays[f,p,z] * model.p_seeding_rate[k,l] * model.v_seeding_machdays[p,k,l,z] for k in model.s_crops) for p in model.s_labperiods)
...
def late_seed_penalty(model,g,k,z):
    ...
    farmer_penalty = sum(sum(model.p_seeding_rate[k,l] * model.v_seeding_machdays[p,k,l,z] * model.p_yield_penalty[p,k,z]
                             for l in model.s_lmus) for p in model.s_labperiods)  * model.p_grainpool_proportion[k,g]

    contract_penalty = sum(sum(model.v_contractseeding_ha[p,k,l,z] * model.p_yield_penalty[p, k, z] for l in model.s_lmus)
...
def stubble_penalty(model,k,s,z):
    ...
    contract_penalty = sum(sum(model.v_contractseeding_ha[p,k,l,z] * model.p_yield_penalty[p,k,z] * model.p_rot_stubble[k,s]
...
def harv_supply(model,k,z):
    ...
    farmer_harv = sum(model.v_harv_hours[p,k,z] * model.p_harv_rate[p,k,z]  for p in model.s_labperiods  )
    contract_harv = model.v_contractharv_hours[k,z] * model.p_contractharv_rate[k]
...
def seeding_cost(model,c,z):
    #contract cost
    contract_cost = sum(sum(sum(model.v_contractseeding_ha[p,k1,l,z] * model.p_contract_seeding_cost[c,z] for l in model.s_lmus) for p in model.s_labperiods) for k1 in model.s_landuses) 
    #cost per ha x number of days seeding x ha per day
    seeding_cost = sum(sum(sum(model.p_seeding_cost[c,l,z] * model.v_seeding_machdays[p,k1,l,z] * model.p_seeding_rate[k1,l] for l in model.s_lmus) for p in model.s_labperiods) for k1 in model.s_landuses)  
...
def harvesting_cost(model,c,z):
    ...
    return sum(model.v_contractharv_hours[k,z] * model.p_contractharv_cost[c,k,z] + sum(model.p_harv_cost[c,k,z] * model.v_harv_hours[p, k, z] for p in model.s_labperiods) for k in model.s_harvcrops)
...
def total_dep(model,z):
    ...
    seeding_depreciation = sum(sum(sum(model.p_seeding_dep[l] * model.v_seeding_machdays[p,k,l,z] * model.p_seeding_rate[k,l] for l in model.s_lmus) for p in  model.s_labperiods) for k in model.s_crops)
    #cost of harv dep = hourly dep x early and late harv hours 
    harv_dep = model.p_harv_dep * sum(sum(model.v_harv_hours[p,k,z] for k in model.s_harvcrops) for p in model.s_labperiods)
