...
def f_con_asset(model):
    ...
    def asset_cost(model,q,s,p7,z9):
        ...
        p7_start = l_p7[0]
        if pe.value(model.p_wyear_inc_qs[q, s]) and pe.value(model.p_mask_season_p7z[p7,z9]):
            return (suppy.f_sup_asset(model,q,s,p7,z9) + macpy.f_mach_asset(model,p7) + stkpy.f_stock_asset(model,q,s,p7,z9) + 8000000) * uinp.finance['opportunity_cost_capital'] \
