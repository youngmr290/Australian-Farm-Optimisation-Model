...
def f1_labpyomo_local(params, model):
    ...
    # Casual supervision
    model.v_casualsupervision_perm = Var(model.s_labperiods, model.s_season_types, bounds=(0,None),
                                         doc='hours of perm labour used for supervision of casual')
    model.v_casualsupervision_manager = Var(model.s_labperiods, model.s_season_types, bounds=(0,None),
    ...
    model.v_quantity_casual = Var(model.s_labperiods, model.s_season_types, bounds=(0,None),
    ...
    model.v_fixed_labour_manager = Var(model.s_labperiods, model.s_worker_levels, model.s_season_types, bounds=(0,None),
    ...
    # labour for fixed activities (this variable transfers labour from source to sink)
    model.v_fixed_labour_permanent = Var(model.s_labperiods, model.s_worker_levels, model.s_season_types, bounds=(0,None),
    ...
    # labour for fixed activities (this variable transfers labour from source to sink)
    model.v_fixed_labour_casual = Var(model.s_labperiods, model.s_worker_levels, model.s_season_types, bounds=(0,None),
    ...
    #param  #
    #########
    model.p_perm_hours = Param(model.s_labperiods, model.s_season_types, initialize= params['permanent hours'], mutable=True, doc='hours worked by a permanent staff in each period')
    ...
    model.p_casual_supervision = Param(model.s_labperiods, model.s_season_types, initialize= params['casual supervision'], mutable=True, doc='hours of supervision required by a casual staff in each period')
    ...
    model.p_casual_lower = Param(model.s_labperiods, model.s_season_types, initialize = params['casual lb'], mutable=True, doc = 'casual availability lower bound')
...
def f_con_casual_bounds(model):
    ...
    def casual_labour_availability(model, p, z):
        return  (model.p_casual_lower[p,z], model.v_quantity_casual[p,z], model.p_casual_upper[p,z]) #pyomos way of: lower <= x <= upper
    model.con_casual_bounds = Constraint(model.s_labperiods, model.s_season_types, rule = casual_labour_availability, doc='bounds the casual labour in each period')
...
def f_con_casual_supervision(model):
    ...
    def transfer_casual_supervision(model,p,z):
        return -model.v_casualsupervision_manager[p,z] - model.v_casualsupervision_perm[p,z] + (model.p_casual_supervision[p,z] * model.v_quantity_casual[p,z]) <= 0
    model.con_casual_supervision = Constraint(model.s_labperiods, model.s_season_types, rule = transfer_casual_supervision, doc='casual require supervision from perm or manager')
...
def f1_casual(model,c0,p7,z):
    return sum(model.v_quantity_casual[p5,z] * model.p_casual_cost[c0,p7,z,p5] for p5 in model.s_labperiods)

def f1_perm(model,c0,p7,z):
    return model.v_quantity_perm * model.p_perm_cost[c0,p7,z]

def f1_manager(model,c0,p7,z):
    return model.v_quantity_manager * model.p_manager_cost[c0,p7,z]

def f_labour_cost(model,c0,p7,z):
    '''
