...
def stubble_all(params):
    ...
    fp_start_p6z = per.f_feed_periods().iloc[:-1].values.astype('datetime64[D]')
    days_since_harv_p6zk = fp_end_p6z[...,na] - harv_date_k
    ...
    for crop, crop_idx in zip(pinp.crop['start_harvest_crops'].index, range(len(pinp.crop['start_harvest_crops']))):
        try:
            stub_foo_harv_k[crop_idx] = base_yields.loc[crop].mean() * stubble_per_grain[(crop,'a')]
            ...
            stub_foo_harv_k[crop_idx] = base_yields.mean()
    ...
    stubble_foo_ks1 = stub_foo_harv_k[:,na] *  (1 - cat_cum_propn_ks1)
    ###adjust for quantity delcine due to deterioration
    stubble_foo_p6zks1 = stubble_foo_ks1 * (1 - quant_decline_p6zk[...,na])
    ###ri availiabilty
    ...
    cons_propn_p6zk = np.clip((fp_len_p6z[...,na] - (fp_end_p6z[...,na] - harv_date_k[na,na,:])) / fp_len_p6z[...,na],0, np.inf)
