        ...
        for f in range(n_feed_periods):
            ...
            for z in range(n_season):
                ...
                for l in [*range(n_lmu)]: #loop through lmu
                    idx = np.searchsorted(i_fxg_foo_oflzt[:,f,l,z,t], grn_foo_start_flzt[f,l,z,t], side='left')   # find where foo_starts fits into the input data
...
def update_reseeding_foo(foo_grn_reseeding_flrzt, foo_dry_reseeding_flrzt,
    ...
    foo_change_lrzt[...,0] += foo_na_lrz  #assuming all non-arable is pasture 0 (annuals)
